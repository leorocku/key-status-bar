...
├── keyboard-hook.js koffi FFI + Win32 钩子封装
...
:: start.bat 期望 electron 装在 %TEMP%（不在项目 node_modules 里）
npm install electron --prefix %TEMP%\electron-temp

:: 运行（非阻塞，经 start.bat 用 %TEMP% 的 electron.exe 加载本目录）
npm start
...
验证方式（无自动化测试）：启动后按任意组合键，确认状态条按键块按“修饰键在前、按下顺序排列”渲染；切换 Caps Lock 看 `CAPS ON/OFF`；托盘右键打开配置面板，改颜色/字号确认状态条即时刷新；拖动状态条后重启确认位置记忆。
...
| 文件 | 角色 |
|---|---|
| `main.js` | 入口；窗口创建、托盘、按键状态机、生命周期（`whenReady`/`will-quit`/单实例锁） |
| `preload.js` | `window.electronAPI` 全部 6 个方法；IPC 白名单唯一出口 |
| `keyboard-hook.js` | koffi/Win32 全部细节隔离在此 |
| `keymap.js` | `getKeyDisplay(vkCode, charCode)`，六级优先链（控制键名 → 小键盘运算符 → 小键盘数字 → 硬编码字符表 → charCode 兜底 → `Key`+hex） |
| `config-store.js` | `DEFAULT_CONFIG`（唯一配置 schema）、`getConfig/setConfig/onConfigChange/flushSave` |
| `renderer/statusbar.js` / `config.js` | 两个窗口的全部逻辑 |
...
- **双 Electron 布局（刻意为之）**：`npm start`/`start.bat` 用 `%TEMP%\electron-temp` 下的 electron；`npm run build`（electron-builder）用项目 `node_modules` 里的。两者独立，别试图“统一”。
...
**测试**：内置 `node --test`，零新增依赖，`npm test` 即可。`tests/keymap.test.js` 覆盖 `getKeyDisplay` 六级优先链与各层 `isModifier` 契约；`tests/config-store.test.js` 用 `require.cache` 注入假 `electron` 模块 + 临时目录做真实磁盘往返（默认值、合并、损坏回落、未知 key 拒绝、`flushSave` 持久化）。无 lint、无 CI，其余验证靠 `npm start` 手动冒烟。
