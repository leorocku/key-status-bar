...
/**
 * @param {object} opts
 * @param {function(): number} opts.getDelay - 返回当前自动释放延迟（毫秒），每次启动计时时读取
 * @param {function(number): void} opts.onForceRelease - 计时到期时回调，参数为 vkCode
 * @param {function} [opts.setTimeout] - 可注入（测试用），默认全局
 * @param {function} [opts.clearTimeout] - 可注入（测试用），默认全局
 * @param {boolean} [opts.enabled] - 初始启停状态，默认开
 */
function createAutoCorrect(opts) {
  const _setTimeout = opts.setTimeout || setTimeout;
  const _clearTimeout = opts.clearTimeout || clearTimeout;

  let enabled = opts.enabled !== false;
  const timers = new Map(); // vkCode -> 计时句柄

  function startTimer(vkCode) {
...
module.exports = { createAutoCorrect, MODIFIER_VKS };
